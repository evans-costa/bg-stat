...
export default function BoardgameInfo({ itemId }) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const newData = await fetchGameData(itemId);
        setData(newData);
      } catch (error) {
        setError(error.message);
  ...
  return (
    <section className="flex flex-col items-center">
      ...
      <form
        ...
        className="boardgame-info flex flex-row gap-3 justify-between"
      >
        <div className="w-fit">
        ...
        <div className="text-slate-200 flex flex-col gap-2 items-center w-3/5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="rounded-md text-gray-100 font-medium bg-sky-600 p-4 my-2"
          >
            {isLoading ? <LoadingSpin /> : "Add to Collection"}
          </button>
        </div>
      </form>
    </section>
